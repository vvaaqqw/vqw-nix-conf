{
  ...
  programs.zsh = {
    ...
    shellAliases = {
      ...
      nix-prx-upd = "z /etc/nixos/nvfpkgs && proxychains4 -f /etc/nixos/misc/prx.conf sudo nvfetcher && z /etc/nixos && proxychains4 -f /etc/nixos/misc/prx.conf sudo nix flake update && proxychains4 -f /etc/nixos/misc/prx.conf nh os boot /etc/nixos && notify-send '❄ upd finished🫠'";
